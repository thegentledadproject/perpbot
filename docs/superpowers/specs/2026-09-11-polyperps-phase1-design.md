...
- **Funding-timestamp convention — checked 2026-09-12, INCONCLUSIVE (TODO).** Convention used everywhere: a `funding_rates` row stamped `T` is the payment settled at `T` for the hour ending `T`, so `Bar(open_ts).funding_rate` = the row at `open_ts + 1h`. Read-only check against `data/polyperps.sqlite3`: all 543 native ticks for instrument 6 (`polymarket_ws`, 2026-09-11T06:33:11–06:34:05Z) carry `funding_rate = 0.0000125`, `next_funding = 2026-09-11T07:00:00Z`. The native `funding_rates` row at 07:00 is not stored (the REST backfill ran at 06:49; latest row 06:00, rate 0.0000125). The nearest row matches, but the rate has been pinned at 0.0000125 for 253 consecutive hours (since 2026-08-30T17:00, last other value 0.00001185), so the match cannot distinguish "hour ending T" from "hour starting T". Circumstantial support for the convention: every native row is stamped `HH:00:00` plus 10–209 ms of jitter, i.e. a settlement event, not a period label; and the ticks name the upcoming settlement (07:00) as `next_funding`. Nothing in the data contradicts the convention; nothing confirms it either. **TODO (before any native `passed` record is approved):** keep the feed running across a settlement at which the rate is *not* pinned, then compare the ticks' `funding_rate`/`next_funding` with the row stamped at that `next_funding`; equal ⇒ confirmed, different ⇒ `build_bars` must look up `funding.get(open_ts)` instead of `open_ts + 1h` and every H1 result must be re-run.
